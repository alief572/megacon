...
$satuan_beli = (!empty($listData[0]->satuan_beli)) ? $listData[0]->satuan_beli : '';
...
<div class="box box-primary">
	<div class="box-body">
		<form id="data_form" method="post" autocomplete="off" enctype='multiple/form-data'>
			...
			<div class="form-group row">
				<div class="col-md-2">
					<label for="">Satuan saat beli</label>
				</div>
				<div class="col-md-7">
					<div class="form-group">
						<select name="satuan_beli" id="" class="form-control input-md">
							<option value="">- Pilih Satuan Packing -</option>
							<?php
							foreach ($list_packing as $item) {
								$selected = '';
								if ($item->id == $satuan_beli) {
									$selected = 'selected';
								}
								echo '<option value="' . $item->id . '" ' . $selected . '>' . ucfirst($item->code) . '</option>';
							}
							?>
						</select>
					</div>
