...
class Confirm_ipp_custom_model extends BF_Model
{
	...
	public function __construct()
	{
		...
		$this->ENABLE_ADD     = has_permission('Confirm_IPP_Custom.Add');
		$this->ENABLE_MANAGE  = has_permission('Confirm_IPP_Custom.Manage');
		$this->ENABLE_VIEW    = has_permission('Confirm_IPP_Custom.View');
		$this->ENABLE_DELETE  = has_permission('Confirm_IPP_Custom.Delete');
	}
	...
	public function get_json_ipp()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$nestedData[]	= "<div align='center'>" . strtoupper(strtolower($row['no_bom'])) . "</div>";
			...
			if ($row['ajukan_sts'] == 'Y') {
				if ($this->ENABLE_MANAGE) {
					$edit		= "&nbsp;<a href='" . site_url($this->uri->segment(1)) . '/add/' . $row['id'] . "' class='btn btn-sm btn-success' title='Approve' data-role='qtip'><i class='fa fa-check'></i></a>";
			...
			if ($row['ajukan_sts'] == 'A') {
				if ($this->ENABLE_MANAGE) {
					if (empty($row['no_bom'])) {
						$edit		= "&nbsp;<a href='" . site_url($this->uri->segment(1)) . '/add_bom/' . $row['id'] . "' class='btn btn-sm btn-primary' title='Add BOM' data-role='qtip'><i class='fa fa-plus'></i></a>";
					} else {
						$edit		= "&nbsp;<a href='" . site_url($this->uri->segment(1)) . '/add_bom/' . $row['id'] . "/" . $row['no_bom'] . "' class='btn btn-sm btn-info' title='Edit BOM' data-role='qtip'><i class='fa fa-edit'></i></a>";
					}
				}
			}
	...
	public function get_query_json_ipp($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		...
		$sql = "SELECT
				...
				FROM
					...
					(SELECT @row:=0) r
				WHERE 1=1 AND a.deleted_date IS NULL AND a.ajukan_sts != 'N' AND 
					(
		...
		$sql .= " ORDER BY a.ajukan_sts_date DESC, " . $columns_order_by[$column_order] . " " . $column_dir . " ";
	...
	public function generate_no_bom_custom()
	{
		$Ym			= date('y');
