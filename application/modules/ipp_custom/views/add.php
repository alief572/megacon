			...
			<div>
				...
				if (!empty($detail)) {
					foreach ($detail as $val => $valx) {
						...
						$standard_k200   		= (!empty($valx['standard_k200'])) ? $valx['standard_k200'] : '';
						$standard_k250   		= (!empty($valx['standard_k250'])) ? $valx['standard_k250'] : '';
						$standard_k300   		= (!empty($valx['standard_k300'])) ? $valx['standard_k300'] : '';
						$standard_k350   		= (!empty($valx['standard_k350'])) ? $valx['standard_k350'] : '';
						$standard_k400   		= (!empty($valx['standard_k400'])) ? $valx['standard_k400'] : '';
						...
						if($standard_k200 !== 'N') {
							$selected_k200 = 'checked';
						}
						if($standard_k250 !== 'N') {
							$selected_k250 = 'checked';
						}
						if($standard_k300 !== 'N') {
							$selected_k300 = 'checked';
						}
						if($standard_k350 !== 'N') {
							$selected_k350 = 'checked';
						}
						if($standard_k400 !== 'N') {
							$selected_k400 = 'checked';
						}
						...
						echo "<tbody class='list_aplikasi_kebutuhan_" . $val . "'>";
						if (!empty($detail_aplikasi_kebutuhan)) {
							$no_apk = 1;
							foreach ($detail_aplikasi_kebutuhan as $item_app) {
								if ($item_app['no_permintaan'] == $val) {
									echo '<tr class="item_aplikasi_kebutuhan_' . $val . '_' . $no_apk . '">';
									echo '<td>';
									echo '<input type="text" class="form-control form-control-sm aplikasi_kebutuhan_' . $val . '" name="aplikasi_kebutuhan[' . $val . '][' . $no_apk . '][nm_aplikasi_kebutuhan]" value="' . $item_app['nama'] . '" readonly>';
									echo '</td>';
									echo '<td class="text-left">';
									if(empty($tanda)) {
										echo '<button type="button" class="btn btn-sm btn-danger" onclick="delApkKeb(' . $val . ',' . $no_apk . ')" title="Delete"><i class="fa fa-trash"></i></button>';
									}
									echo '</td>';
									echo '</tr>';
						...
						if (empty($tanda)) {
							...
							echo "<tr>";
							echo "<td>";
							echo "<input type='text' class='form-control form-control-sm aplikasi_kebutuhan_new_" . $val . "'>";
							echo "</td>";
							echo "<td class='text-left'>";
							if(empty($tanda)) {
								echo "<button type='button' class='btn btn-sm btn-success w-100 add_aplikasi_kebutuhan' data-id='" . $val . "'><i class='fa fa-plus'></i> Add</button>";
							}
							echo "</td>";
							echo "</tr>";
							echo "</tbody>";
						...
						echo "		<div class='checkbox'><label><input type='checkbox' name='Detail[" . $val . "][standard_k200]' value='Y' ".$selected_k200.">K200</label></div>";
						echo "		<div class='checkbox'><label><input type='checkbox' name='Detail[" . $val . "][standard_k250]' value='Y' ".$selected_k250.">K250</label></div>";
						echo "		<div class='checkbox'><label><input type='checkbox' name='Detail[" . $val . "][standard_k300]' value='Y' ".$selected_k300.">K300</label></div>";
						echo "		</div>";
						echo "	</div>";
						echo "	<div class='col-md-2'>";
						echo "		<div class='form-group'><label></label>";
						echo "		<div class='checkbox'><label><input type='checkbox' name='Detail[" . $val . "][standard_k350]' value='Y' ".$selected_k350.">K350</label></div>";
						echo "		<div class='checkbox'><label><input type='checkbox' name='Detail[" . $val . "][standard_k400]' value='Y' ".$selected_k400.">K400</label></div>";
						echo "		</div>";
						echo "	</div>";
						echo "</div>";
						...
						echo "			<th class='text-center' width='30%'>Height</th>";
						...
						foreach ($getdetailProduct4 as $key => $value) {
							...
							echo "</td>";
							echo "<td align='left'>";
							echo "<input type='text' name='Detail[" . $val . "][ukuran_jadi][" . $new_number . "][height]' class='form-control input-md text-center autoNumeric4' value='" . $value['height'] . "'>";
							echo "</td>";
							echo "<td align='left'>";
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(document).on('click', '.add_aplikasi_kebutuhan', function() {
			var id = $(this).data('id');

			var no = $('.aplikasi_kebutuhan_' + id).length;
			no += 1;

			var nm = $('.aplikasi_kebutuhan_new_' + id).val();
			if (nm !== '') {
				var hasil = '<tr class="item_aplikasi_kebutuhan_' + id + '_' + no + '">';
				hasil += '<td>';
				hasil += '<input type="text" class="form-control form-control-sm aplikasi_kebutuhan_' + id + '" name="aplikasi_kebutuhan[' + id + '][' + no + '][nm_aplikasi_kebutuhan]" value="' + nm + '" readonly>';
				hasil += '</td>';
				hasil += '<td class="text-left">';
				hasil += '<button type="button" class="btn btn-sm btn-danger" onclick="delApkKeb(' + id + ',' + no + ')" title="Delete"><i class="fa fa-trash"></i></button>';
				hasil += '</td>';
				hasil += '</tr>';

				$('.list_aplikasi_kebutuhan_' + id).append(hasil);
			} else {
				swal({
					type: 'warning',
					title: 'Warning !',
					text: 'Please make sure to fill the input !'
				});
			}
		});
	...
	function delApkKeb(no_header, no) {
		// alert(no_header);
		// alert(no);
		$('.item_aplikasi_kebutuhan_' + no_header + '_' + no).remove();
	}
</script>