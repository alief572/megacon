...
class Bom_hi_grid_custom_model extends BF_Model
{
	...
	public function __construct()
	{
		...
		$this->ENABLE_ADD     = has_permission('BOM_HI-Grid_Custom.Add');
		$this->ENABLE_MANAGE  = has_permission('BOM_HI-Grid_Custom.Manage');
		$this->ENABLE_VIEW    = has_permission('BOM_HI-Grid_Custom.View');
		$this->ENABLE_DELETE  = has_permission('BOM_HI-Grid_Custom.Delete');
	}
		...
		$sql = "SELECT
				...
				WHERE 1=1 AND a.deleted_date IS NULL AND a.category = 'grid custom' AND 
					(
						...
						OR c.no_ipp LIKE '%" . $this->db->escape_like_str($like_value) . "%'
						OR c.project LIKE '%" . $this->db->escape_like_str($like_value) . "%'
						OR d.nm_customer LIKE '%" . $this->db->escape_like_str($like_value) . "%'
					)
		";
		// echo $sql; exit;
