...
class Request_pr_material_model extends BF_Model
{
  ...
  //START NEW QUERY DATA NEW
  public function get_data_json_reorder_point_new()
  {
    // $controller			= ucfirst(strtolower($this->uri->segment(1)));
    // $Arr_Akses			= getAcccesmenu($controller);
  ...
  public function query_data_json_reorder_point_new($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
  {
    $tanggal  = date('Y-m-d', strtotime('-1 month', strtotime(date('Y-m-d'))));
    $bulan    = ltrim(date('m', strtotime($tanggal)), '0');
    $tahun    = date('Y', strtotime($tanggal));
