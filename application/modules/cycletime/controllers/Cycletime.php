...
class Cycletime extends Admin_Controller
{
  ...
  public function edit()
  {
    ...
    $machine  = $this->db->query("SELECT a.* FROM asset a JOIN rate_machine b ON b.kd_mesin = a.kd_asset WHERE a.category='2' AND a.deleted_date IS NULL GROUP BY SUBSTR(a.kd_asset, 1, 20) ORDER BY a.nm_asset ASC ")->result_array();
    $mould  = $this->db->query("SELECT a.* FROM asset a JOIN rate_mold b ON b.kd_mesin = a.kd_asset WHERE a.category='1' AND a.deleted_date IS NULL GROUP BY SUBSTR(a.kd_asset, 1, 20) ORDER BY a.nm_asset ASC ")->result_array();
