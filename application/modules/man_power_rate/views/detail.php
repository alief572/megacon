...
// print_r($header);
$total_direct   	= (!empty($header)) ? $header[0]->total_direct : 0;
$total_bpjs   		= (!empty($header)) ? $header[0]->total_bpjs : 0;
$total_biaya_lain   = (!empty($header)) ? $header[0]->total_biaya_lain : 0;

$rate_dollar   = (!empty($header)) ? $header[0]->rate_dollar : 0;
$upah_per_bulan_dollar   = (!empty($header)) ? $header[0]->upah_per_bulan_dollar : 0;
$upah_per_jam_dollar   = (!empty($header)) ? $header[0]->upah_per_jam_dollar : 0;
$upah_per_bulan   = (!empty($header)) ? $header[0]->upah_per_bulan : 0;
$upah_per_jam   = (!empty($header)) ? $header[0]->upah_per_jam : 0;

$kurs_tanggal = (!empty($header)) ? date('d-M-Y', strtotime($header[0]->kurs_tanggal)) : ''; 
				...
				<table class='' width='60%'>
					...
					<tbody>
						...
						if (!empty($detail_direct)) {
							$SUM_DIRECT = 0;
							foreach ($detail_direct as $val => $valx) {
								$val++;
						...
						if (!empty($detail_bpjs)) {
							...
							echo "</tr>";
							$SUM_BPJS = 0;
							foreach ($detail_bpjs as $val => $valx) {
								$val++;
								$SUM_BPJS += $valx['nilai'];
								echo "<tr>";
								echo "<td align='center'>" . $val . "</td>";
						...
						if (!empty($detail_lain)) {
							...
							echo "</tr>";
							$SUM_LAINNYA = 0;
							foreach ($detail_lain as $val => $valx) {
								$val++;
						...
						echo "<th class='text-right text-red'>" . $kurs_tanggal . "</th>";
						echo "</tr>";
						echo "<tr>";
						echo "<th class='text-center'></th>";
						echo "<th class='text-left'>Rate MP</th>";
